...
class TestChatStateManager:
    ...
    def test_add_assistant_message(self, state_manager):
        ...
        msg = state_manager.add_assistant_message(
            "Response", thinking="Thinking...", reasoning_details={"details": "test"}
        )
    ...
    def test_import_conversation_validates_roles(self, state_manager):
        """Test that invalid roles are rejected during import."""
        data = {
            "messages": [
        ...
        assert result is True
        assert len(state_manager.messages) == 2
        assert state_manager.messages[0]["role"] == "user"
        assert state_manager.messages[1]["role"] == "assistant"

    def test_import_conversation_validates_content_size(self, state_manager):
        """Test that oversized messages are rejected."""
        data = {"messages": [{"role": "user", "content": "A" * 100_001}]}
    ...
    def test_import_conversation_skips_non_dict_messages(self, state_manager):
    ...
    def test_import_conversation_no_messages_key(self, state_manager):
        """Test import fails without messages key."""
    ...
    def test_import_conversation_validates_content_type(self, state_manager):
        """Test that non-string content is rejected."""
        data = {"messages": [{"role": "user", "content": 12345}]}
    ...
    def test_import_from_json(self, state_manager):
        ...
        json_str = (
            '{"session_id": "test", "messages": [{"role": "user", "content": "Hi"}]}'
        )
